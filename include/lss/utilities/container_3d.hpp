...
using lmatrix_3d = lss_containers::lmatrix_3d;
using lmatrix_3d_ptr = lss_containers::lmatrix_3d_ptr;
...
struct lmatrix_3d_builder
{
  ...
  public:
    LSS_API explicit lmatrix_3d_builder();

    LSS_API lmatrix_3d_builder &rows(std::size_t rows);

    LSS_API lmatrix_3d_builder &columns(std::size_t columns);

    LSS_API lmatrix_3d_builder &layers(std::size_t layers);

    LSS_API lmatrix_3d_builder &default_value(double value);

    LSS_API lmatrix_3d_ptr build();
};
