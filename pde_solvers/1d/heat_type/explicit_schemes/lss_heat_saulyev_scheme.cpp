#include "lss_heat_saulyev_scheme.hpp"

#include "../../../../boundaries/lss_dirichlet_boundary.hpp"
#include "../../../../boundaries/lss_neumann_boundary.hpp"
#include "../../../../boundaries/lss_robin_boundary.hpp"
#include "../solver_method/lss_heat_saulyev_solver_method.hpp"
...
void heat_saulyev_scheme::initialize(heat_coefficients_ptr const &coefficients)
{
    ...
    s_coeffs_ = std::make_shared<heat_saulyev_coefficients>(coefficients);
}

heat_saulyev_scheme::heat_saulyev_scheme(heat_coefficients_ptr const &coefficients,
...
heat_saulyev_scheme::~heat_saulyev_scheme()
{
}

void heat_saulyev_scheme::operator()(container_t &solution, bool is_heat_sourse_set,
                                     ...
                                     traverse_direction_enum traverse_dir)
{
    ...
    auto const &solver_method_ptr =
        std::make_shared<heat_saulyev_solver_method>(s_coeffs_, grid_cfg_, is_heat_sourse_set);
...
void heat_saulyev_scheme::operator()(container_t &solution, bool is_heat_sourse_set,
                                     std::function<double(double, double)> const &heat_source,
                                     traverse_direction_enum traverse_dir, container_2d<by_enum::Row> &solutions)
{
    auto const &timer = discretization_cfg_->time_range();
    const double k = discretization_cfg_->time_step();
    // last time index:
