...
void double_sweep_boundary::initialise(boundary_2d_pair const &boundary, double time, double space_args)
{
    const auto a = std::get<0>(low_quad_);
    const auto b = std::get<1>(low_quad_);
    const auto c = std::get<2>(low_quad_);
...
const double double_sweep_boundary::lower_boundary(boundary_2d_pair const &boundary, double time, double space_args)
{
    double ret{};
