...
void thomas_lu_solver_boundary::initialise(boundary_2d_pair const &boundary, double time, double space_args)
{
    const auto a_0 = std::get<0>(lowest_quad_);
    const auto b_0 = std::get<1>(lowest_quad_);
    const auto c_0 = std::get<2>(lowest_quad_);
...
void thomas_lu_solver_boundary::finalise(boundary_2d_pair const &boundary, double time, double space_args)
{
    const auto a = std::get<0>(higher_quad_);
    const auto b = std::get<1>(higher_quad_);
    const auto c = std::get<2>(higher_quad_);
...
const double thomas_lu_solver_boundary::upper_boundary(boundary_2d_pair const &boundary, double time, double space_args)
{
    double ret{};
    auto const second_bnd = boundary.second;
    if (auto ptr = std::dynamic_pointer_cast<dirichlet_boundary_2d>(second_bnd))
...
const double thomas_lu_solver_boundary::lower_boundary(boundary_2d_pair const &boundary, double time, double space_args)
{
    double ret{};
