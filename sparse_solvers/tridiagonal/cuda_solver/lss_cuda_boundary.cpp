...
cuda_boundary::cuda_boundary(const std::size_t discretization_size, const double &space_step)
...
void cuda_boundary::set_lowest_quad(const quad_t &lowest_quad)
{
    lowest_quad_ = lowest_quad;
}

void cuda_boundary::set_lower_quad(const quad_t &lower_quad)
{
    lower_quad_ = lower_quad;
}

void cuda_boundary::set_highest_quad(const quad_t &highest_quad)
{
    highest_quad_ = highest_quad;
}

void cuda_boundary::set_higher_quad(const quad_t &higher_quad)
{
    higher_quad_ = higher_quad;
}

const triplet_t cuda_boundary::init_coefficients(boundary_1d_pair const &boundary, double time)
{
    initialise(boundary, time);
    return std::make_tuple(b_init_, c_init_, f_init_);
}

const triplet_t cuda_boundary::init_coefficients(boundary_2d_pair const &boundary, double time, double space_arg)
{
    initialise(boundary, time, space_arg);
    return std::make_tuple(b_init_, c_init_, f_init_);
}

const triplet_t cuda_boundary::final_coefficients(boundary_1d_pair const &boundary, double time)
{
    finalise(boundary, time);
    return std::make_tuple(a_end_, b_end_, f_end_);
}

const triplet_t cuda_boundary::final_coefficients(boundary_2d_pair const &boundary, double time, double space_arg)
{
    finalise(boundary, time, space_arg);
    return std::make_tuple(a_end_, b_end_, f_end_);
}

std::size_t cuda_boundary::start_index() const
{
    return start_index_;
}
std::size_t cuda_boundary::end_index() const
{
    return end_index_;
}

void cuda_boundary::initialise(boundary_1d_pair const &boundary, double time)
{
...
void cuda_boundary::initialise(boundary_2d_pair const &boundary, double time, double space_args)
{
    const auto a_0 = std::get<0>(lowest_quad_);
    const auto b_0 = std::get<1>(lowest_quad_);
    const auto c_0 = std::get<2>(lowest_quad_);
...
void cuda_boundary::finalise(boundary_1d_pair const &boundary, double time)
{
...
void cuda_boundary::finalise(boundary_2d_pair const &boundary, double time, double space_args)
{
    const auto a = std::get<0>(higher_quad_);
    const auto b = std::get<1>(higher_quad_);
    const auto c = std::get<2>(higher_quad_);
...
const double cuda_boundary::upper_boundary(boundary_1d_pair const &boundary, double time)
{
    double ret{};
...
const double cuda_boundary::upper_boundary(boundary_2d_pair const &boundary, double time, double space_args)
{
    double ret{};
...
const double cuda_boundary::lower_boundary(boundary_1d_pair const &boundary, double time)
{
    double ret{};
...
const double cuda_boundary::lower_boundary(boundary_2d_pair const &boundary, double time, double space_args)
{
    double ret{};
